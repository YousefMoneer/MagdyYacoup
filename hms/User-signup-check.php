...
if (isset($_POST['uname']) && isset($_POST['password'])
	...
	if (empty($uname)) {
		header("Location: user-signup.php?error=User Name is required&$user_data");
	    exit();
	}else if(empty($pass)){
        ...
        header("Location: user-signup.php?error=Re Password is required&$user_data");
	    exit();
	}

	else if(empty($name)){
        header("Location: user-signup.php?error=Name is required&$user_data");
	    exit();
	}
		...
		if (mysqli_num_rows($result) > 0) {
			header("Location: users-signup.php?error=The username is taken try another&$user_data");
	        exit();
		}else {
